...
function decoratedata(details) {
  ...
  let data = `<div class="p-6 bg-blue-950 rounded-xl shadow-md border border-blue-800 flex flex-col md:flex-row gap-6 items-center md:items-start text-left">
    ...
    details.login
  }" class="h-24 w-24 rounded-full border border-blue-700 object-cover" />
            <div>
              ...
              <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-blue-300 mb-4">
                <div><span class="font-semibold text-white">Followers:</span> ${
                  details.followers
                }</div>
                <div><span class="font-semibold text-white">Following:</span> ${
                  details.following
                }</div>
                <div><span class="font-semibold text-white">Public Repos:</span> ${
                  details.public_repos
                }</div>
                <div><span class="font-semibold text-white">Location:</span> ${
                  details.location || "N/A"
                }</div>
                <div><span class="font-semibold text-white">Company:</span> ${
                  details.company || "N/A"
                }</div>
                <div><span class="font-semibold text-white">Blog:</span> ${
                  details.blog
              ...
              <a href="${
                details.html_url
              }" target="_blank" class="inline-block mt-2 text-blue-400 hover:underline">
                🔗 View GitHub Profile
              </a>
            </div>
          </div>`;
